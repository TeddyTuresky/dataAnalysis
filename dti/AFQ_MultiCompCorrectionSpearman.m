function [alphaFWE, statFWE, clusterFWE, stats] = AFQ_MultiCompCorrectionSpearman(data,y,alpha,method,cThresh)
        ...
        switch(stattest)
            case('corr')
                for ii = 1:nperm
                    ...
                    [stat(ii,:) p(ii,:)] = corr(y, data(rows,:),'rows', 'pairwise','type','Spearman'); % TKT added, 2020
                end
            case('ttest')
                for ii = 1:nperm
                    rows = Shuffle(y);
