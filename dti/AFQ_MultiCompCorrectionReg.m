function [alphaFWE, statFWE, clusterFWE, stats] = AFQ_MultiCompCorrectionReg(data,y,alpha,method,cThresh,reg)
...
% Tract Diffusion Profiles, values at nearby nodes are highly correlated
        ...
        switch(stattest)
            case('corr')
                for ii = 1:nperm
                    ...
                    rows1 = Shuffle(1:size(data,1));
                    rows2 = Shuffle(1:size(data,1));
                    [stat(ii,:) p(ii,:)] = partialcorr(y, data(rows1,:), reg(rows2,:), 'rows', 'pairwise'); % TKT added, 2020
                end
            case('ttest')
                for ii = 1:nperm
                    rows = Shuffle(y);
