...
stepGNG = '/Volumes/DMC-Gaab2/tools/tkt_tools/BEAN/spmBatches/GNGtemp.mat';
stepLSM = '/Volumes/DMC-Gaab2/tools/tkt_tools/BEAN/spmBatches/LSMtemp.mat';
stepSTORY = '/Volumes/DMC-Gaab2/tools/tkt_tools/BEAN/spmBatches/STORYtemp.mat';
levGNG = '/Volumes/DMC-Gaab2/tools/tkt_tools/BEAN/spmBatches/GNG1lev.mat';
...
a = strsplit(strtrim(ls('-d',[fullfile(path1,file) '/*/*/*'])),{'\t','\n'});
...
E = dir2([dep1 '/' m{i,1} '/f*.nii']);
for ii = 1:n_img
    matlabbatch{1,1}.spm.temporal.st.scans{1,1}{ii,1} = [dep1 '/' m{i,1} '/' E(ii).name];
end
...
for iii = 1:(n_img - disc)
    ...
    matlabbatch{1,2}.spm.spatial.realign.estwrite.data{1,1}{iii,1} = [dep1 '/' m{i,1} '/a' E(iii+disc).name];
    matlabbatch{1,3}.spm.spatial.coreg.estimate.other{iii,1} = [dep1 '/' m{i,1} '/ra' E(iii+disc).name];    
    matlabbatch{1,4}.spm.util.defs.out{1,1}.pull.fnames{iii,1} = [dep1 '/' m{i,1} '/ra' E(iii+disc).name];
    matlabbatch{1,5}.spm.spatial.smooth.data{iii,1} = [dep1 '/' m{i,1} '/wra' E(iii+disc).name];
...
H{2,:} = ls([dep1 '/' m{h(v,1),1} '/mri/wms*.nii']); % warped struct
H{3,:} = ls([dep1 '/' m{i,1} '/wraf*00050*.nii']); % warped func
H{4,:} = ls([dep1 '/' m{i,1} '/swraf*00050*.nii']); % warped, smoothed func
...
if n_lsm > 0 && n_wm > 0 && lv(end) > 0 && wv(end) > 0
    while u2 == 0
        ...
        u2 = u2+1;
    load(levLSM);
    L = dir2([dep1 '/' m{lv(1,1),1} '/swraf*.nii']);
    W = dir2([dep1 '/' m{wv(1,1),1} '/swraf*.nii']);

    mkdir([dep1 '/' m{lv(1,1),1} '/stats']);
    matlabbatch{1,1}.spm.stats.fmri_spec.dir{1,1} = [dep1 '/' m{lv(1,1),1} '/stats'];    
    matlabbatch{1,1}.spm.stats.fmri_spec.sess(1).multi_reg{1,1} = [dep1 '/' m{lv(1,1),1} '/multipleRegressFile.mat'];
    matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).multi_reg{1,1} = [dep1 '/' m{wv(1,1),1} '/multipleRegressFile.mat'];

    for iv = 1:(n_img - disc)
        matlabbatch{1,1}.spm.stats.fmri_spec.sess(1).scans{iv,1} = [dep1 '/' m{lv(1,1),1} '/' L(iv).name];
        matlabbatch{1,1}.spm.stats.fmri_spec.sess(2).scans{iv,1} = [dep1 '/' m{wv(1,1),1} '/' W(iv).name];

    end

    matlabbatch{1,2}.spm.stats.fmri_est.spmmat{1,1} = [dep1 '/' m{lv(1,1),1} '/stats/SPM.mat'];
...
if n_story > 0 && sv(end) > 0
    ...
    load(levSTORY);
    mkdir([dep1 '/' m{qsv(1,1),1} '/stats']);
    matlabbatch{1,1}.spm.stats.fmri_spec.dir{1,1} = [dep1 '/' m{qsv(1,1),1} '/stats'];
    
    for s = 1:numel(qsv)
        
    S = dir2([dep1 '/' m{qsv(1,s),1} '/swraf*.nii']);      
    matlabbatch{1,1}.spm.stats.fmri_spec.sess(s) = matlabbatch{1,1}.spm.stats.fmri_spec.sess(1);   
    matlabbatch{1,1}.spm.stats.fmri_spec.sess(s).multi_reg{1,1} = [dep1 '/' m{qsv(1,s),1} '/multipleRegressFile.mat'];

    for iv = 1:(n_img - disc)
        matlabbatch{1,1}.spm.stats.fmri_spec.sess(s).scans{iv,1} = [dep1 '/' m{qsv(1,s),1} '/' S(iv).name];
    end
    
    clearvars S s iv
    end
    
    matlabbatch{1,2}.spm.stats.fmri_est.spmmat{1,1} = [dep1 '/' m{qsv(1,1),1} '/stats/SPM.mat'];
