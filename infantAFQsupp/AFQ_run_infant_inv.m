function [afq patient_data control_data norms abn abnTracts] = AFQ_run_infant_inv(sub_dirs, sub_group, afq)
...
if AFQ_get(afq, 'use ANTS')
    afq = AFQ_ComputeSpatialNormalization_infant(afq);
end
...
for ii = runsubs
    ...
    if AFQ_get(afq, 'do segmentation',ii) == 1
        ...
        % Segment fiber group
        fg_classified = AFQ_SegmentFiberGroups_infant_inv(dtFile, fg, [], [],[], antsInvWarp);
