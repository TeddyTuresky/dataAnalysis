function [fg_classified,fg_unclassified,classification,fg] = ...
    AFQ_SegmentFiberGroups_infant_inv(dt6File, fg, Atlas, ...
...
% (c) Vistalab
%
% Added some parameters for infants. 
% theodore.turesky@childrens.harvard.edu, 2019
...
% TKT (2019) commented out this section --------------------------------------
...
moriLoc = fullfile(tdir, Atlas);
moriTracts = readFileNifti(moriLoc);
...
%--------------------------------------------------------------------------------
% TKT (2019) added below from ANTS normalization in earlier step of AFQ_run_infant
%--------------------------------------------------------------------------------

% necessary elements for downstream ANTS implementation
xformInf2MNILoc = '/Users/cinnamon/Documents/transformInf2MNI';
affineInf2MNI = fullfile(xformInf2MNILoc,'unc2mni0GenericAffine.mat');
invAffineInf2MNI = ['[' affineInf2MNI ',1]' ];
warpInf2MNI = fullfile(xformInf2MNILoc,'unc2mni1Warp.nii.gz');
invWarpInf2MNI = fullfile(xformInf2MNILoc,'unc2mni1InverseWarp.nii.gz');

[pinv,~] = fileparts(antsInvWarp);
affineXform = [pinv '/t1_acpc0GenericAffine.mat'];
invAffineXform = ['[' pinv '/t1_acpc0GenericAffine.mat,1]'];

fibs = dir2(fullfile(xformInf2MNILoc,'inMori','fib*.nii'));
refNifti = fullfile(xformInf2MNILoc,'inMori',fibs(1).name);
moriWarps = moriTracts;
moriWarps.data = moriWarps.data.*0;

for f = 1:size(fibs,1)
    invar = fullfile(xformInf2MNILoc,'inMori',fibs(f).name);
    outf = fullfile(pinv,['ind-' fibs(f).name]);
    ANTS_applyTransformsToMori(invar, refNifti, invWarpInf2MNI, invAffineInf2MNI, antsInvWarp, invAffineXform, outf);
    inffib = readFileNifti(outf);
    moriWarps.data(:,:,:,f) = inffib.data;
    clearvars invar outf inffib
end


moriWarps.data(:,:,:,15) = moriWarps.data(:,:,:,15)-moriWarps.data(:,:,:,19);
moriWarps.data(:,:,:,16) = moriWarps.data(:,:,:,16)-moriWarps.data(:,:,:,20);
%--------------------------------------------------------------------------------

% fg_sn = dtiXformFiberCoords(fg, invDef); 
% Changing fiber transform, TKT (2019)

% ************************************************************************
% need to affine align points after warp from native to infant. need to go
% from infant to adult. then need to go from adult to mori fiber. currently
% using mni_icbm152_t2_tal_nlin_asym_09a template, but may need to switch
% to MNI_JHU because these are aligned closely with mori. !!update:
% MNI_JHU_T2 works *ok* with N4_nihpd template affine transform: antsRegistration 
% -d 3 -o [jhu2inf,jhu2inf_Warped.nii.gz,jhu2inf_InverseWarped.nii.gz] 
% -m MI[MNI_JHU_T2.nii.gz,N4_nihpd_asym_02-05_t1w.nii.gz,1,32,Regular, 0.25] 
% -c [100x70x50x20,1e-6,10] -t Affine[0.1] -f 8x4x2x1 -s 3x2x1x0
% !!update: might want to use mrAnatXformCoords function for affine
% adjustment after applyTransformsToPoints command
% also need to see whether we need to convert to LPS:
% https://sourceforge.net/p/advants/discussion/840261/thread/4d4ec44b/
% https://sourceforge.net/p/advants/discussion/840261/thread/dcb66a3a/
% https://sourceforge.net/p/advants/discussion/840261/thread/9fbbaab7/?limit=25#1783
% https://github.com/netstim/leaddbs/blob/master/helpers/ea_antsmat2mat.m
% is the fixed matrix in the affine *.mat a mapping to world space? see
% page 6 of ants2.pdf
% maybe it's just an origin difference as ANTs uses this information.
% it's possible too that all of this is a problem with how AFQ renders
% slices to the fiber meshes - i.e., may be using header info
% ************************************************************************
...
% the following transform uses the affine function at the bottom of
% mrAnatXformCoords
fgCoords = mrAnatXformCoords(moriWarps.qto_ijk, horzcat(fg.fibers{:}));

fgCoords2 = {};
for ll = 1:size(fg.fibers,1); 
    fgCoords2{ll,1} = mrAnatXformCoords(moriWarps.qto_ijk, fg.fibers{ll,1})'; 
end

save([pinv '/xformFibsAtlas-inv.mat'],'fgCoords');
save([pinv '/xformFibsAtlas2-inv.mat'],'fgCoords2');
...
for(ii=1:sz(4))
    ...
    p = myCinterp3(double(moriWarps.data(:,:,:,ii))/100, sz([1,2]), sz(3), fgCoords(:,[2,1,3]));
    ...
    for roiID=1:size(moriRois, 1)
        ...
        if recomputeROIs
            ...
            if exist('antsInvWarp','var') && ~isempty(antsInvWarp)
                ...
                roi1(roiID) = ANTS_CreateRoiFromMniNifti_infant(ROI_img_file, [], refNifti, invAffineInf2MNI, invWarpInf2MNI, invAffineXform, antsInvWarp, outfile);
            else
        ...
        if recomputeROIs
            ...
            if exist('antsInvWarp','var') && ~isempty(antsInvWarp)
                ...
                roi2(roiID) = ANTS_CreateRoiFromMniNifti_infant(ROI_img_file, [], refNifti, invAffineInf2MNI, invWarpInf2MNI, invAffineXform, antsInvWarp, outfile);
            else
...
% equal to the number of fibers in fg_classified + fg_unclassified. To
