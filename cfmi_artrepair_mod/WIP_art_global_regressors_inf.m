...
% ------------------------ 
%   % Rotation measure assumes voxel is 40 mm from origin of rotation.
    if realignfile == 1 | realignfile == 0
        ...
        for i = 2:nscans
            delta(i,1) = (mv_data(i-1,1) - mv_data(i,1))^2 +...
                    ...
                    .27*(mv_data(i-1,4) - mv_data(i,4))^2 +...
                    .27*(mv_data(i-1,5) - mv_data(i,5))^2 +...
                    .27*(mv_data(i-1,6) - mv_data(i,6))^2;
...
plot(delta);
ylabel('Motion (mm/TR)');
xlabel('Scan to scan movement (~mm). Rotation assumes 40 mm from origin');
y_lim = get(gca, 'YLim');
