...
for h = 1:length(hand);
    ...
    for g = 1:length(group);
        ...
        for j = 1:length(subj);
            for i = 1:n_img
                matlabbatch{1,j}.spm.spatial.smooth.data{i,1} = strrep(matlabbatch{1,j}.spm.spatial.smooth.data{i,1},'conped',long);
                matlabbatch{1,j}.spm.spatial.smooth.data{i,1} = strrep(matlabbatch{1,j}.spm.spatial.smooth.data{i,1},'cp01',[group1 subj(j,:)]);
