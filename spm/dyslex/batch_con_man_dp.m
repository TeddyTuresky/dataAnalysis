...
hand = ['L';'R']; 
group = ['cp';'dp';'ca'];
step = {'9ConMan-rpRMSartGlobal'};
%     {'9ConMan-ArtGlobal'; '9ConMan-ArtGlobal_offs';...
%     '9ConMan-rpArtGlobal'; '9ConMan-rpArtGlobal_offs';...
%     '9ConMan-rpdispArtGlobal'; '9ConMan-rpdispArtGlobal_offs';...
%     '9ConMan-trndisprotdispArtGlobal'; '9ConMan-trndisprotdispArtGlobal_offs'};
    ...
    for h = 1:length(hand);
        ...
        for g = 1:length(group);
            ...
            for j = 1:length(subj);
                ...
                k = j - p;
                matlabbatch{1,k}.spm.stats.con.spmmat{1,1} = strrep(matlabbatch{1,k}.spm.stats.con.spmmat{1,1},'conped',long);
                matlabbatch{1,k}.spm.stats.con.spmmat{1,1} = strrep(matlabbatch{1,k}.spm.stats.con.spmmat{1,1},'cp01',[group1 subj(j,:)]);
                matlabbatch{1,k}.spm.stats.con.spmmat{1,1} = strrep(matlabbatch{1,k}.spm.stats.con.spmmat{1,1},'motorL',['motor' hand1]);
                matlabbatch{1,k}.spm.stats.con.consess{1,1}.tcon.name = strrep(matlabbatch{1,k}.spm.stats.con.consess{1,1}.tcon.name,'L',hand1);
            end
