...
for h = 1:length(hand);
    ...
    for g = 1:length(group);
        ...
        for j = 1:length(subj);
            matlabbatch{1,j}.spm.util.defs.comp{1,1}.def{1,1} = strrep(matlabbatch{1,j}.spm.util.defs.comp{1,1}.def{1,1},'conped',long);
            matlabbatch{1,j}.spm.util.defs.comp{1,1}.def{1,1} = strrep(matlabbatch{1,j}.spm.util.defs.comp{1,1}.def{1,1},'cp01',[group1 subj(j,:)]);
            for i = 1:n_img
                matlabbatch{1,j}.spm.util.defs.out{1,1}.pull.fnames{i,1} = strrep(matlabbatch{1,j}.spm.util.defs.out{1,1}.pull.fnames{i,1},'conped',long);
                matlabbatch{1,j}.spm.util.defs.out{1,1}.pull.fnames{i,1} = strrep(matlabbatch{1,j}.spm.util.defs.out{1,1}.pull.fnames{i,1},'cp01',[group1 subj(j,:)]);
