...
hand = ['L';'R']; 
group = ['cp';'dp';'ca'];
step = {'8ModEst-rpRMSartGlobal'};
%     {'8ModEst-ArtGlobal'; '8ModEst-ArtGlobal_offs';...
%     '8ModEst-rpArtGlobal'; '8ModEst-rpArtGlobal_offs';...
%     '8ModEst-rpdispArtGlobal'; '8ModEst-rpdispArtGlobal_offs';...
%     '8ModEst-trndisprotdispArtGlobal'; '8ModEst-trndisprotdispArtGlobal_offs'};
    ...
    for h = 1:length(hand);
        ...
        for g = 1:length(group);
            ...
            for j = 1:length(subj);
                ...
                k = j - p;
                matlabbatch{1,k}.spm.stats.fmri_est.spmmat{1,1} = strrep(matlabbatch{1,k}.spm.stats.fmri_est.spmmat{1,1},'conped',long);
                matlabbatch{1,k}.spm.stats.fmri_est.spmmat{1,1} = strrep(matlabbatch{1,k}.spm.stats.fmri_est.spmmat{1,1},'cp01',[group1 subj(j,:)]);
                matlabbatch{1,k}.spm.stats.fmri_est.spmmat{1,1} = strrep(matlabbatch{1,k}.spm.stats.fmri_est.spmmat{1,1},'motorL',['motor' hand1]); 
            end
