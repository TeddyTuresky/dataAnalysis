...
for h = 1:length(hand);
    ...
    for g = 1:length(group);
        ...
        for j = 1:length(subj);
            matlabbatch{1,j}.spm.stats.fmri_est.spmmat{1,1} = strrep(matlabbatch{1,j}.spm.stats.fmri_est.spmmat{1,1},'adults',long);
            matlabbatch{1,j}.spm.stats.fmri_est.spmmat{1,1} = strrep(matlabbatch{1,j}.spm.stats.fmri_est.spmmat{1,1},'a01',[group1 subj(j,:)]);
            matlabbatch{1,j}.spm.stats.fmri_est.spmmat{1,1} = strrep(matlabbatch{1,j}.spm.stats.fmri_est.spmmat{1,1},'motorL',['motor' hand1]); 
        end
